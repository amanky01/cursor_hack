...
const CounsellorLoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await loginCounsellor(email, password);
      if (!response.success) {
  ...
  return (
    <Layout title="Counsellor Login - Sehat-Saathi" description="Counsellor sign-in">
      <div className={styles.authContainer}>
        <div className={styles.authCard}>
          <div className={styles.authHeader}>
            <h1 className={styles.title}>Counsellor Sign In</h1>
            <p className={styles.subtitle}>Counsellor access portal</p>
          </div>
